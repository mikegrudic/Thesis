 ...
 */
 int quartic(double dd[5], double sol[4], double soli[4], int* Nsol)
 {
	...
	*Nsol = 0;
	...
	zsol = - 1.e99;
	for (i=0;i<ncube;i++)	zsol = std::max(zsol, z[i]);	//Not sure C has std::max fct
	z[0] =zsol;
	xK2 = 2.0*z[0] -p;
	xK = sqrt(xK2);
	xL = q/(2.0*xK);
...
int cubic(double A[4], double X[3], int* L)
{
	...
	// ====deterstd::mine the degree of the polynomial ====
	...
	if (A[3] != 0.0)
	{
		...
		if ( DIS < 0.0 )
		{
			...
			PHI = acos(std::min(1.0,std::max(-1.0,Q/sqrt(-P))));
 ...
 /*
  *     ==== perform one step of a newton iteration in order to std::minimize
  *          round-off errors ====
  */
	for (i=0;i<*L;i++)
	{
