...
#pragma omp parallel
{
  double rootsr[4], rootsi[4], coeffs[5];
  int i;
...
#pragma omp for private(i)
  for (i = 0; i < nn; i++){
    ...
    coeffs[4] = C1;
    coeffs[3] = 2.0;
    coeffs[2] = aSqr*C1 - L*L - Q;
    coeffs[1] = 2*(pow(L - a*E, 2) + Q);
    coeffs[0] = -aSqr*Q;

    int info;
    quartic(coeffs, rootsr, rootsi, &info);
    //    std::cout << rootsr[0] << std::endl << rootsr[1] << std::endl <<rootsr[2] << std::endl << rootsr[3] << std::endl;
